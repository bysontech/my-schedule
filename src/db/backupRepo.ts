...
export async function exportAll(): Promise<BackupJson> {
  const [tasks, groups, projects, buckets] = await Promise.all([
    db.tasks.toArray(),
    db.groups.toArray(),
    db.projects.toArray(),
    db.buckets.toArray(),
  ...
  return {
    ...
    data: { tasks, groups, projects, buckets },
  };
}
...
export async function importAll(backup: BackupJson): Promise<void> {
  await db.transaction("rw", db.tasks, db.groups, db.projects, db.buckets, async () => {
