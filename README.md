...
Focus をキーボードだけで回せるように強化:

- **タイトル検索**: Focus / Planning 両方に検索バー追加
- **キーボードショートカット** (Focus):
  - `/` → 検索フォーカス
  - `Esc` → 検索クリア / 解除
  - `j` / `k` → タスク選択を上下移動
  - `Space` → 選択タスクの完了トグル
  - `Enter` → 選択タスクの todo ↔ in_progress 切替
- **選択ハイライト**: 選択中の行を枠+背景色で強調
- 入力欄フォーカス中はショートカット暴発しない
- Planning 検索: タイトル + メモで絞込 + `/` / `Esc`

### Cycle-10: TaskRow共通化 & ドロワー編集

UI構造を統一し、操作を整理:

- **TaskRow**: 全画面共通のタスク行コンポーネント（チェック / 優先度アイコン / タイトル / 期限）
- **完了表現統一**: 取り消し線 + 薄色 + 薄背景（`taskrow--done`）
- **期限色ルール**: overdue / today は期限文字のみ赤（行全体は赤くしない）
- **KebabMenu（…）**: 編集/削除の常時ボタンを廃止し、…メニューに集約
- **右ドロワー編集**: Dashboard のタスクタイトルクリック → TaskEditDrawer で編集 → 保存 → 即反映
  - 編集項目: title / dueDate / priority / status / group / project / buckets / memo
  - Esc でドロワーを閉じる
